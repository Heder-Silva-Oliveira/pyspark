...
status = parametro.filter(parametro.COD == 2).select(parametro.VAR_STATUS).first()[0]
mes_atual = int((datetime.now().strftime('%m'))) #verificar ano/mes
ano_atual = int(datetime.now().strftime('%Y'))
mes_passado = (mes_atual - 3)
...
print(type(dt_atual))
if  status == 0:
        if data_hoje >= '2023-08-30':  #entra dt_atual que vem do parametro
                #DIARIO
